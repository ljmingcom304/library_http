...
import com.ljming.http.Network;
import com.ljming.http.parse.HttpCallBack;
import com.ljming.http.parse.HttpHeader;
import com.ljming.http.parse.HttpMode;
import com.ljming.http.parse.Resolver;
    ...
    public <T> void request(final Class<T> clazz, final HttpCallBack<T> callBack) {
        ...
        if (method == HttpMode.GET) {
            ...
            if (uploadFile != null) {
                if (uploadFile.exists()) {
                    ...
                    Log.e(TAG, uploadFile.getName() + " does not exist.");
                }
            } else {
                if (params.containsKey(TYPE_JSON)) {//JSON类型的请求
                    ...
                    for (Map.Entry<String, String> entry : params.entrySet()) {
                        ...
                        if (key != null && value != null) {
                            ...
                            Log.e(TAG, "请求参数为空：key=" + key + " value=" + value);
                        }
                    }
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                Log.i(TAG, "Url:" + printUrl);
                Log.i(TAG, "###################################################################");
                Log.i(TAG, call.request().url().toString());
                Log.i(TAG, "Status:[Method:" + request.method() + "][Code:" + response.code() + "][Message:" + response.message() + "]");
                Log.i(TAG, "Request:" + headersToString(reqHeaders));//请求
                String reqCookie = cookiesToString(request.url(), reqHeaders);
                if (!TextUtils.isEmpty(reqCookie)) {
                    Log.i(TAG, "Cookie-Request:" + reqCookie);
                }
                String resCookie = cookiesToString(request.url(), resHeaders);
                if (!TextUtils.isEmpty(resCookie)) {
                    Log.i(TAG, "Cookie-Response:" + resCookie);
                }
                Log.i(TAG, "Response:" + headersToString(resHeaders));//响应
                Log.i(TAG, "###################################################################");
                String result = "";
